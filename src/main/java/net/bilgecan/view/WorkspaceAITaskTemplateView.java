...
@Route(value = "wsaitask", layout = MainLayout.class)
@RolesAllowed({"USER", "ADMIN"})
public class WorkspaceAITaskTemplateView extends VerticalLayout implements HasDynamicTitle {
    ...
    public WorkspaceAITaskTemplateView(AITaskTemplateService aiTaskTemplateService, AITaskRunService aiTaskRunService, OllamaModelRegistry ollamaModelRegistry,
        ...
        add(new H2(translations.t("aiTaskTemplate.workspaceAITaskTemplates")));
        ...
        paginationView = new PaginationView() {
            ...
            protected Page loadAndGetPage(Pageable pageable, String searchTerm) {
                Page<AITaskDto> termPage = aiTaskTemplateService.findInWorkspacePaginated(pageable, searchTerm, currentMembership.getWorkspaceId());
                // Update the grid with the current page data
                aiTaskGrid.setItems(termPage.getContent());
                return termPage;
            }
        };
        paginationView.setCurrentPageKey("currentPageWorkspaceAITaskTemplate");
        add(paginationView);
        initWorkspace();
    ...
    private void initWorkspace() {
        ...
        if (!memberships.isEmpty()) {
            ...
            saveButton.setEnabled(WorkspacePermissions.can(currentMembership.getRole(), Permission.EDIT));
        } else {
    ...
    private void runTask(AITaskDto aiTaskDto) {
        ...
        dialog.addConfirmListener(event -> {
            aiTaskRunService.saveTaskRunInWorkspace(aiTaskDto.getId(), currentMembership.getWorkspaceId());
    ...
    private void deleteAITaskConfirmationDialog(AITaskDto aiTask) {
        ...
        dialog.addConfirmListener(event -> {
            aiTaskTemplateService.deleteInWorkspace(aiTask, currentMembership.getWorkspaceId());
    ...
    private boolean saveAITask() {
        try {
            ...
            aiTaskTemplateService.saveWorkspaceAITask(currentAITask, currentMembership.getWorkspaceId());
    ...
    public String getPageTitle() {
        return translations.t("menu.workspaceTasks");
    }
}
